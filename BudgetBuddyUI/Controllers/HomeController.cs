    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (defaultBudgetId > 0)
            {
                ...
                List<OverviewModel> overviewModels = new List<OverviewModel>();
                overviewModels.Add(overviewModel);
                overviewModels.Add(projectionModel);

                OverviewPageModel overviewPageModel = new OverviewPageModel()
                {
                    OverviewModels = overviewModels
                };

                return View(overviewPageModel);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> Index(OverviewPageModel pageOverview)
        {
            // Figure out who is logged in, and get the AspNetUserId for identifying the user
            // in the BudgetDataDb
            var aspNetUserId = await _userManager.GetUserIdAsync(await _userManager.GetUserAsync(User));
            ...
            if (defaultBudgetId > 0)
            {
                ...
                foreach (var item in newMonthlySummaries)
                {
                    item.IncomeAmount += item.IncomeAmount * (pageOverview.InflationPercentage / 100);
                    item.ExpenseAmaount += item.ExpenseAmaount * (pageOverview.InflationPercentage / 100);
                    item.MarginAmount += item.MarginAmount * (pageOverview.InflationPercentage / 100);
                }
